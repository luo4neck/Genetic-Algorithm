...
int fitness_GA(GRP* p,int j,int i)
{
int year1=0, year2=0, rslt=0;
check_GA(p,i);
...
MPI_Status stat;
GRP* p;
int i,j,itr,g,n,A,B,tmp,son,mut_p, myid, nprocs, rslt[1], in[1];
...
for(i=0;i<itr;i++)
  ...
  if(myid == 0)
    ...
    for(j=1;j<n;j++)// sending message here..
      {
      in[0] = j-1;
      MPI_Send(in, 1, MPI_INT, j, 0, MPI_COMM_WORLD);
      
      MPI_Recv(rslt, 1, MPI_INT, j, 0, MPI_COMM_WORLD, &stat);
      p->his[j-1] = p->his[j-1] + (double)rslt[0] * 0.05;
  ...
  {
    MPI_Recv(in, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &stat);

    rslt[0] = fitness_GA(p,in[0],p->his[ in[0] - 1 ]);

    MPI_Send(rslt, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
  }
