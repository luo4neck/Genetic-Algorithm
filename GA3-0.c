...
MPI_Status stat;
GRP* p;
int i,j,itr,g,n,A,B,tmp,son,mut_p, myid, nprocs, in[1];
...
//    p->his[j-1] = p->his[j-1] + (double)rslt * 0.05;
    }
  }
  else
  {
  MPI_Recv(in, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &stat);
  fitness_GA(p,in[0],p->his[ in[0] - 1 ]);
  }

/*  for(j=0;j<n+1;j++)//fitness function..
    {
      if(j==0)
      {
      MPI_Send();
